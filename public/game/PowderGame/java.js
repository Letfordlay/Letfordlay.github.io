...
function cookie_get(nm) {
    ...
    if (document.cookie) {
        ...
        for (i = 0; i < r.length; i++) r[i] = decodeURIComponent(r[i]);
    }
...
function mb_link(u, s) {
    ...
    if (navigator.userAgent.match(/iPhone|iPad|iPod|Android|Kindle/))
        h.innerHTML =
            ...
            u +
            'm/\'" STYLE="FONT-SIZE:40px;LINE-HEIGHT:42px;"><BR><BR>' +
            h.innerHTML;
}
...
function counter(a) {
    ...
    w =
        '<DIV STYLE="CLEAR:both;WIDTH:180px;HEIGHT:50px;OVERFLOW:hidden;BORDER-BOTTOM:1px solid #600"><DIV STYLE="WIDTH:200px">';
    for (i = 0; i < 8; i++) {
        h = 50 - (50 * a[i]) / m;
        w +=
            '<DIV STYLE="FLOAT:left;WIDTH:' +
            (i == 7 ? "7" : "11") +
            "px;HEIGHT:50px;MARGIN:" +
            h +
            "px 5px 0 6px;" +
            (i == 7 ? "BORDER:2px solid " : "BACKGROUND:") +
            '#d72"></DIV>';
    }
    w += "</DIV></DIV>";
...
function dust_4works(a) {
    ...
    for (i = 0; i < 7 * 4; i += 7) {
        ...
        if (a[29] == 1)
            w +=
                '<IMG SRC="/images/dust/' +
                Math.floor(a[i + 1] / 1000) * 1000 +
                "/" +
                a[i + 1] +
                '.gif" WIDTH="100" HEIGHT="75">';
        else
            w +=
                '<IMG SRC="/images/dust2/' +
                Math.floor(a[i + 1] / 1000) +
                "/" +
                a[i + 1] +
                '.png" WIDTH="124" HEIGHT="70">';
        w += a[i + 2] + "<BR>";
        w += "by " + a[i + 0] + "<BR>";
        w += '<DIV CLASS="star">';
        if (a[i + 3] < 10)
            w +=
                '<DIV CLASS="c0" STYLE="LEFT:' +
                (Math.round((a[i + 3] - 0) * 6.11) - 55) +
                'px;"></DIV>';
        else if (a[i + 3] < 50)
            w +=
                '<DIV CLASS="c1" STYLE="LEFT:' +
                (Math.round((a[i + 3] - 9) * 1.37) - 55) +
                'px;"></DIV>';
        else if (a[i + 3] < 100)
            w +=
                '<DIV CLASS="c2" STYLE="LEFT:' +
                (Math.round((a[i + 3] - 49) * 1.12) - 55) +
                'px;"></DIV>';
        else if (a[i + 3] < 500)
            w +=
                '<DIV CLASS="c3" STYLE="LEFT:' +
                (Math.round((a[i + 3] - 99) * 0.12) - 55) +
                'px;"></DIV>';
...
function more_write(cmt, nm, pt, tm, lg) {
    ...
    if (pt < 10) v += '<DIV CLASS="c0" STYLE="LEFT:' + ((pt - 0) * 6.11 - 55) + 'px"></DIV>';
        ...
        v += '<DIV CLASS="c2" STYLE="LEFT:' + ((pt - 49) * 1.122 - 55) + 'px"></DIV>';
    else if (pt < 500)
        v += '<DIV CLASS="c3" STYLE="LEFT:' + ((pt - 99) * 0.137 - 55) + 'px"></DIV>';
    else v += '<DIV CLASS="c4"></DIV>';
    v +=
        "<SPAN></SPAN></DIV>" +
        pt +
        (lg == "0" ? "票" : lg == "1" ? "votes" : "投票") +
        "<BR>" +
        tm +
        "</DIV>";
...
function hu_tag(id, lg) {
    var w = "",
        ck = cookie_get("user");
    var v = [
        ["復活の呪文", "Save game", "保存游戏"],
        ["ゲット", "Get", "载入"],
        ["セット", "Set", "保存"],
        ["ユーザー登録が必要です。", "User registration is required.", "需要用户注册。"]
    ];
    ...
    if (ck[3] != "") {
        ...
        w +=
            ...
            ')" ONMOUSEDOWN="';
        w +=
            "document.getElementById('bun').value='';document.getElementById('pass').innerHTML='　';";
        w += '"> ';
        w +=
            '<INPUT TYPE="SUBMIT" VALUE="' +
            v[2][lg] +
...
function load(id, lg) {
    document.getElementById("bun").value = document
        .getElementById("bun")
        .value.replace(/\x0D\x0A|\x0D|\x0A/g, "");
...
function post(u_no, v_no, v) {
    ...
    for (var k in post_v[v_no]) v[k] = post_v[v_no][k];
    for (var k in v) {
...
function facebook(u, t) {
    ...
    var d =
        '<A HREF="http://www.facebook.com/plugins/like.php?href=' +
        u +
        '" REL="nofollow" TARGET="_blank" STYLE="BACKGROUND-POSITION:center -66px"></A>';
    d +=
        '<A HREF="https://www.facebook.com/sharer/sharer.php?u=' +
        u +
        '" REL="nofollow" TARGET="_blank" STYLE="BACKGROUND-POSITION:center -44px"></A>';
    d +=
        ...
        u +
        '" REL="nofollow" TARGET="_blank" STYLE="BACKGROUND-POSITION:center -22px"></A>';
